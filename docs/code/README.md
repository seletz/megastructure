...
| Placement and walking | [[placement]] | `SectorMultiMesh`, which places solved sectors as one MultiMesh per tile mesh with merged or chunked collision built on the worker, `SectorStreamer`, which loads and frees sectors around the player, `SectorGridMap` kept for debugging, the walk scene and the player capsule, and their checks. |
