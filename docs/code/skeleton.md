...
`show_graph` and `marker_size` setters mark the viewer dirty, the per-kind
toggles only hide a mesh. The lines are drawn after all boxes (render
...
one `show_<kind>` toggle per edge kind and `marker_size`; below them one `grammar …` section per `@export_group` of
