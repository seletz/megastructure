...
| `run-walk` | import | Runs the world walk scene at seed 0: the 3³ sectors around the origin solved on worker threads, placed as GridMaps as they arrive, and a capsule to walk them; F free fly, V first or third person ([[placement]]). |
...
| `check` | import, check-scripts, smoke, panel-check, skeleton-histogram, skeleton-stats, graph-check, graph-connectivity --quick, raster-check --quick, tileset-check, adjacency-check, tiles-check-fixtures, tileset-build-check, tiles-check-placeholder --quick, solver-check --quick, boundary-check --quick, gridmap-check, walk-check | The pull request tier, under 2 minutes: the statistical tasks run on their reduced sample ([[#Check tiers]]). Then runs `jobs-check --quick` on its own (`mise run --skip-deps`), because it times the main thread and fails when the parallel tasks take its CPU, and loads the project headlessly in editor mode and quits. CI runs it on every pull request ([[ci-and-export]]). |
| `check-full` | the same tasks without `--quick` | The full tier, about 10 minutes: every statistical task at full size, then `jobs-check` on its own and the same headless editor load. CI runs it on pushes to `develop` and nightly. |
...
| `walk-check` | import | Walks the `Player` capsule through a `SectorGridMap` one physics step per frame (`--fixed-fps 60`) and fails when it falls 0.5 m below the floor, does not reach a cell within 240 frames or ends over 1 m from the goal. Default: a hand-laid course of placeholder tiles; `mise run walk-check --sector x,y,z [--seed N]` solves a real sector and walks portal to portal along the edge rasteriser's records ([[placement]]). Headless; the course is part of `check`. |
...
| [solver_sector_run.gd](../../scripts/tools/solver_sector_run.gd) | `solver-check`, `solver-sector` | `class_name SolverSectorRun`: one pipeline pass for a real sector (type, records, `SectorDomains.build`, `solve`, reusing a given solver) with `broken_records`, `bad_adjacencies` and the family `histogram` (tiles of no family by prototype name). |
...
| [walk_check.gd](../../scripts/tools/walk_check.gd) | `walk-check` | Parses `[--sector x,y,z] [--seed N]`. Without a sector lays the course into a 24³ result (air, floor and stair cells with solid under each and at each stair's high end); with one, solves it with `SectorJobs.solve_sector` and builds the route from `EdgeRasteriser.rasterise` (first kept horizontal edge reversed, then the last one). Places the cells with `SectorGridMap`, adds a `Player`, settles 30 frames and steers `scripted_direction` at each cell centre; prints the route with tiles, frames per cell, steps climbed, the final distance, and on failure the tiles in the columns at and ahead of the feet. |
