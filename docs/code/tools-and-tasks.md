...
| `solver-check` | import | Checks `SectorSolver` on the placeholder tileset: unconstrained 8³ solves at seeds 0 to 4 succeed, repeat byte for byte on a second instance, allow every adjacency and match [[solver_reference_seed0]]; the entropy flag and `domains` restrictions work. Prints the tile histogram, steps and time per seed and the time of the first successful 24³ solve among seeds 0 to 4; `--update` rewrites the reference ([[solver]]). Headless; part of `check`. |
...
| [solver_check.gd](../../scripts/tools/solver_check.gd) | `solver-check` | Builds the placeholder `TileLibrary`; for seeds 0 to 4 solves an 8³ grid at sector (0, 0, 0) on two `SectorSolver` instances and requires success, identical cells and steps, and no face-neighbour pair `is_allowed` rejects, printing steps, queue pops, time, a SHA-256 prefix and the tile histogram. Repeats the check once with `use_entropy`. Restricts one cell to solid through `domains` and requires it in the result, then restricts a floor directly above air and requires a contradiction. Compares the seed 0 SHA-256 of `cells.to_byte_array()` with the `sha256:` line of [[solver_reference_seed0]], or rewrites that file with `--update`. Then solves 24³ at seeds 0 to 4 until one succeeds, printing each time and checking the adjacencies of the solved one; a contradiction there is only reported. |
