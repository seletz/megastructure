...
In this project: `resources/tilesets/placeholder.tres`, 40 prototypes
...
A cell beside a record that, besides the sector's fill tile, admits the free
tiles the record needs there, such as the rock a catwalk hangs on.

In this project: `SectorDomains.support_cells`; on the placeholder tileset
only catwalks and ladders end up with support, a `backing` plate
([[0018-walk-tiles-only-in-record-cells]], #180). They replaced the
full-height support columns of decision #158.
...
The head room over a tunnel through solid rock: open in the lowest 1.8 m of
the cell so a walker's head fits, rock above and on the closed sides.

In this project: the `vault` prototypes of the placeholder tileset (straight,
corner, T, cross and end), with `stairwell` and `stairwell_end` over a stair
in rock; headroom records take them in solid sectors.

See also: [[tileset#The placeholder tileset]], Tunnel edge.
