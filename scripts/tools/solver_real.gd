	...
	for seed in seeds:
		...
		for type: Skeleton.SectorType in [Skeleton.SectorType.STRATUM, Skeleton.SectorType.SOLID]:
			...
			if type == Skeleton.SectorType.STRATUM and min_solved >= 0 and counts.solved < min_solved:
				short += 1
			rows.append("  seed %d, %d %s: %d solved, %d degraded, %d failed in an attempt, %d rejected by the record checks, %d failed on the starting propagation" % [
				seed, sectors.size(), Skeleton.type_name(type), counts.solved, counts.degraded, counts.failed, counts.rejected, counts.propagation,
			])
	for row in rows:
		print(row)
...
## Builds, propagates and (with `solve`) solves one sector of `type`,
## printing a line and counting its outcome in `counts`.
func _run_sector(library: TileLibrary, rasteriser: EdgeRasteriser, solver: SectorSolver, seed: int, sector: Vector3i, type: Skeleton.SectorType, solve: bool, counts: Dictionary) -> void:
	var records := rasteriser.records_for_sector(sector)
	var n := rasteriser.graph.cells_per_sector()
	var size := Vector3i(n, n, n)
	var label := "seed %d %s %s" % [seed, Skeleton.type_name(type), sector]
	var built := SectorDomains.build(library, size, type, records)
	if not built.error.is_empty():
		counts.rejected += 1
		print("  %s: %d records, rejected: %s" % [label, records.size(), built.error])
		return
	var empty := SolverSectorRun.empty_cell(library, size, built.words)
	if empty >= 0:
		counts.propagation += 1
		print("  %s: %d records, empties cell %s; minimal records:" % [label, records.size(), SolverSectorRun.position(size, empty)])
		for line in _minimal(library, size, type, records):
			print("      %s" % line)
		return
	if not solve:
		return
...
func _minimal(library: TileLibrary, size: Vector3i, type: Skeleton.SectorType, records: Array[EdgeRasteriser.Record]) -> PackedStringArray:
	...
	while true:
		...
		while k < kept.size():
			...
			if _fails(library, size, type, trial):
				kept = trial
			else:
				k += chunk
		if chunk == 1:
...
static func _fails(library: TileLibrary, size: Vector3i, type: Skeleton.SectorType, records: Array[EdgeRasteriser.Record]) -> bool:
	var built := SectorDomains.build(library, size, type, records)
